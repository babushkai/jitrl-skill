...
const store = new store_js_1.ExperienceStore();
    ...
    try {
        switch (name) {
            case "jitrl_search": {
                ...
                const k = args?.k || 5;
                const stepCount = args?.step_count || 0;
            ...
            case "jitrl_store": {
                const experience = await store.add(args?.state, args?.action, args?.outcome, args?.trajectory_context, args?.future_rewards);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "jitrl_inject": {
                ...
                const advantages = store.getAdvantages(results);
                const injection = (0, injection_js_1.generateInjection)(results, advantages);
                return {
                    content: [
                        {
                            type: "text",
